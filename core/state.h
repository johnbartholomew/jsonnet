...
struct HeapClosure : public HeapEntity {
    ...
    const BuiltinFunctionBody *builtinBody() const {
        if (this->body->type == AST_BUILTIN_FUNCTION_BODY) {
            return static_cast<const BuiltinFunctionBody*>(this->body);
        } else {
            return nullptr;
        }
    }
    bool isBuiltin() const { return builtinBody() != nullptr; }
};
